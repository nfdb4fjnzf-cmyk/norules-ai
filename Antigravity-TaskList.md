...
# NoAI 系統優化規劃（安全性＋營收強化）

> 本文件用於給 Google AI / 開發團隊參考，作為 NoAI 後續 V3+ 的系統設計說明。

---

## 1. 儲存空間限制（Storage Quota）

- **目標**
  - 控制每位用戶可使用的雲端儲存空間（提示紀錄、產生的圖片 / 影片 / 文件）。
  - 避免惡意大量佔用空間造成成本與資安風險。

- **實作重點**
  - 依方案（輕用量 / 中用量 / 企業級）給予不同 Storage 上限。
  - 超額時禁止新檔案寫入，改為提示「刪除舊檔或升級方案」。
  - 後端週期性（例：每日）掃描與壓縮舊檔；對過期檔案做自動清除。

- **安全性關聯**
  - 避免過量檔案成為潛在敏感資料堆積點。
  - 明確的生命周期管理（保留天數＋刪除機制），降低資料外洩風險。

---

## 2. 模型成本控管（Token Cost Management）

- **目標**
  - 控制 LLM Token 消耗，確保每位用戶使用量與收費一致。
  - 避免高風險用戶透過超長提示 / 迴圈呼叫消耗大量資源。

- **實作重點**
  - 每次呼叫前先預估 Token（提示＋預期回覆），超過上限則拒絕或要求縮短內容。
  - 將 Token 使用量記錄入用戶「計費紀錄 / 活動紀錄」中。
  - 搭配扣點邏輯：超量時「暫停使用」而不是無限透支。

- **安全性關聯**
  - 避免系統被惡意刷 Token，變相 DoS。
  - 透過明細記錄，可追查異常使用行為。

---

## 3. 併發流量限制（Concurrency Control）
...
- **目標**
  - 透過 CDN 加速靜態資源（前端、圖片、CSS、JS）與部分 API Cache。
  - 降低原始伺服器負載，讓攻擊變得更難成功。

- **實作重點**
  - 使用 Cloudflare / CDN 進行資源 Cache。
  - 對公開文件 / Demo 頁面做強 Cache，避免每次打到 Origin。
  - 排除帶有個人資料、金流資訊的 API，不做 Cache。

- **安全性關聯**
  - 減少 Origin 的暴露面，被打爆前先擋在 CDN Edge。
  - 搭配 WAF 規則過濾常見攻擊（SQLi、XSS、暴力嘗試）。
...
- **目標**
  - 保證在資料庫或檔案系統出現問題時，可以快速恢復。
  - 降低因錯誤部署或攻擊導致的全面停擺風險。

- **實作重點**
  - 設定每日 / 每小時備份（Database + Storage Metadata）。
  - 備份存放在獨立專案或區域（避免一同被攻擊刪除）。
  - 定期做「復原演練」，實際從備份啟動測試環境。

- **安全性關聯**
  - 即使遭駭或誤刪，仍可回復到最近版本。
  - 備份需加密＋限制存取權限，避免成為敏感資料外流源。
...
- **目標**
  - 讓代理商或使用者可透過推薦碼推廣 NoAI，獲得分潤。
  - 增加營收，鼓勵合法推廣。

- **實作重點**
  - 推薦碼綁定推廣者帳號，記錄每位被推薦者的訂閱與消費。
  - 從訂閱與點數購買中抽出一定比例分潤。
  - 後台提供報表：推廣數量、轉換率、收益。

- **安全性關聯**
  - 避免「假帳號洗推薦」：限制同 IP / 同裝置註冊數。
  - 加入風險檢測：短時間大量註冊自動進入審核或凍結。
...
- **目標**
  - 針對違規用戶（洗帳、詐騙、過量攻擊）能快速停權。
  - 保護正常用戶、降低金流與法遵風險。

- **實作重點**
  - 設計「黑名單標記」欄位：email、IP、裝置指紋等。
  - 自動規則：超出風險門檻 → 先暫停功能 → 通知管理員審核。
  - 後台可手動「解除 / 延長停權」，並留下操作紀錄。

- **安全性關聯**
  - 形成一條龍防護：異常偵測 → 自動限制 → 管理員處理。
  - 降低系統被長期利用做灰產 / 非法用途。
...
- **目標**
  - 讓一個主帳號（公司）可以開多個子帳號（同事）。
  - 分權管理：誰可以查看金流、誰可以只使用模型。

- **實作重點**
  - 角色權限設計：Owner / Admin / Editor / Viewer。
  - 每個角色有不同可見範圍（訂閱、扣點紀錄、生成紀錄）。
  - 所有操作記錄（生成、刪除、設定修改）可追蹤到「哪個子帳號」。

- **安全性關聯**
  - 避免所有員工都看到金流或敏感資料。
  - 發生問題時可追責到具體帳號，而非整個公司模糊責任。

---

## 9. 影片生成排程（Queue System）

- **目標**
  - 對高成本的影片生成（如 Sora / 影片 API）使用隊列排程。
  - 避免瞬間大量請求造成成本爆炸或服務掛掉。

- **實作重點**
  - 先扣除或鎖定預估點數，再排入隊列。
  - 每位用戶同時最多 N 支排程，其餘排隊等待。
  - 提供「排隊狀態」查詢（等待中 / 進行中 / 失敗 / 完成）。

- **安全性關聯**
  - 阻擋腳本式濫用：「一鍵產生上百支影片」的惡意行為。
  - 清楚記錄每一支影片請求，方便事後稽核。

---

## 10. 模型版本管理（LLM Version Control）

- **目標**
  - 支援多種模型版本（例如 GPT-4.x / GPT-5.x / Gemini 等）。
  - 在更新模型或 Prompt 邏輯時不影響舊用戶正常運作。

- **實作重點**
  - 對每次呼叫記錄「使用哪個模型版本」。
  - 支持「鎖定版本」的企業級客戶，避免突然行為改變。
  - 更新前先在沙盒環境測試：風險評估 / 成本評估。

- **安全性關聯**
  - 避免因模型更新導致的誤判（例如合規內容被錯誤放行）。
  - 可以回溯：發生問題時，知道是哪個版本造成的。

---

## 11. 自動重試邏輯（Retry Policy）

- **目標**
  - 處理暫時性的 API 失敗（超時、網路問題）時，自動重試。
  - 避免單純因為網路抖動就造成扣點卻沒產出結果。

- **實作重點**
  - 設定「重試次數＋間隔時間」，例如最多重試 2 次。
  - 僅對特定錯誤碼重試（5xx / Timeout），避免無限打錯誤請求。
  - 記錄每次重試狀態，供後台檢查問題。

- **安全性關聯**
  - 避免重試機制被利用成放大攻擊（需搭配併發限制）。
  - 確保扣點邏輯與重試邏輯一致，不會重複扣點。

---

## 12. 系統健康監測（Health Check & Monitoring）

- **目標**
  - 即時掌握系統狀態（延遲、錯誤率、API 成功率）。
  - 在異常發生前就收到警報，降低停機時間。

- **實作重點**
  - 為各個服務設計 Health Check Endpoint。
  - 導入監控與警報（如：平均延遲、錯誤率、流量峰值）。
  - 設定門檻：超出即通知工程 / 老闆，如 Email / Telegram Bot。

- **安全性關聯**
  - 異常高流量可能代表攻擊行為，監測可提前發現。
  - 快速處理異常可減少用戶資料暴露時間。

---

## 13. 自動生成落地頁（Landing Page Generator）

> 此功能為 NoAI 優先開發的重要模組，與上述 12 點共同提升「安全性＋轉化＋營收」。

### 13.1 功能概念

- 使用者在 NoAI 內輸入：
  - 產業類別（例：博弈、電商、在地店家、App 推廣）。
  - 目標國家與語言。
  - 行銷目標（留資料、加 LINE / TG、下載 App、註冊試玩）。
- NoAI 自動產生：
  - 一頁式落地頁 HTML / CSS。
  - Meta / TikTok / GA 等追蹤碼位置預留。
  - 行動呼叫按鈕（CTA）＋ 表單 / 按鈕與轉跳連結。

### 13.2 自動掛子網域與部署流程（Cloudflare Pages 優先方案）

1. 使用者在 NoAI 後台設定主網域（例：`noai-landing.com`）。
2. NoAI 生成落地頁時：
   - 自動建立子網域（例：`ph-7spin-2025.noai-landing.com`）。
   - 將靜態檔案部署至 Cloudflare Pages。
   - 自動設定 DNS CNAME 指向該專案。
3. 後台顯示：
   - 落地頁網址。
   - 部署狀態（建立中 / 完成 / 失敗）。
   - 檔案大小與版本編號。
...
- **隔離性**
  - 每一個落地頁專案與主站程式邏輯隔離，攻擊落地頁不會直接影響 NoAI 主系統。
  - 不在落地頁中放任何管理後台或金流 API 金鑰。

- **資料保護**
  - 若落地頁有表單（收集 email、Line ID 等），資料直接送往 NoAI 安全 API，不在前端存明文。
  - 所有傳輸使用 HTTPS，強制跳轉。

- **內容合規**
  - 生成前由 LLM 做一次「合規檢查」（博弈／金融等敏感產業），降低違規風險。
  - 提供「敏感字清單」，被觸發時提醒用戶調整文案。

### 13.4 收費與營收模型

- **訂閱內含額度**
  - 不同方案內含「可生成落地頁數量」與「可啟用的同時落地頁數」。
  - 例如：
    - 輕用量：每月 1–2 個落地頁。
    - 中用量：每月 5–10 個落地頁。
    - 企業級：不限次數，但有總流量上限。
...
- **額外加購**
  - 超出方案額度時，可單筆購買「新增落地頁名額」或「額外流量」。
  - 可搭配推薦碼 / 代理制度，把落地頁當作高轉化賣點（協助代理推廣）。

### 13.5 操作與權限

- **團隊權限**
  - Owner / Admin 可建立與刪除落地頁。
  - Editor 只能編輯文案與素材，不能刪除專案。
  - Viewer 只可查看成效報表與連結，不可改動內容。

- **成效與監控**
  - 每個落地頁記錄：
    - 瀏覽數、點擊數、轉化數。
    - 來源國家、流量來源（廣告 / 自然流量）。
  - 異常流量（例如短時間大量機器人造訪）觸發告警，並可自動啟動：
    - 啟用 CAPTCHA。
    - 封鎖特定 IP / 國家。
...
你是一套「廣告素材風險分析系統」的核心模型，負責針對使用者提供的素材、文案與落地頁網址，生成 Facebook / TikTok / Google 三個平台的風險分析報告。

請依照以下規格輸出報告內容：

【輸入內容】
1. Report Language（繁體中文、簡體中文、英文）
2. Material（圖片或影片）
3. Copywriting（廣告文案）
4. Landing Page URL（落地頁）

【你的任務】
請根據上方四項輸入，產生一份「跨三平台的風險分析報告」，內容需包含：

----------------------------------
【A. Facebook Meta Ads 審核分析】
----------------------------------
- 危險程度（0～100 分）
- 違規點逐項列出（含政策名稱）
- 素材中涉及的敏感元素（文字、畫面、人物、構圖）
- 可能導致 Limited Learning 或拒登的原因
- 提供具體可執行的「過審建議」
...
----------------------------------
- 危險程度（0～100 分）
- 是否涉及 Google Core Policies 禁止內容
- 涉及品牌、版權、商標、醫療、金融等高風險區塊
- 落地頁是否違規（內容、跳轉、體驗、真實性）
- 提供具體可執行的「過審建議」

----------------------------------
【D. Landing Page 分析（若有 URL）】
----------------------------------
- 是否存在重定向、跳轉、下載誘導
- 是否包含賭博、成人、暴力、金融高風險內容
- 是否存在假倒數、假獎勵、欺騙性導引
- 提供符合 Meta / Google 共同政策的改善建議
- 提供 UX/轉化優化建議
...
1. Facebook 風險評分（分數）
2. Facebook 詳細風險分析與建議
3. TikTok 風險評分（分數）
4. TikTok 詳細風險分析與建議
5. Google 風險評分（分數）
6. Google 詳細風險分析與建議
7. Landing Page 分析（若有）
8. 文案合規改寫（可直接使用）
9. 素材修改建議（圖片/影片）
