...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        if (!id) {
            return res.status(400).json(errorResponse(ErrorCodes.BAD_REQUEST, 'Generation ID is required'));
        }

        const generation = await historyService.getGeneration(id as string, user.uid);

        return res.status(200).json(successResponse(generation));
