...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        // External Mode cannot use Image Analyzer
        ...
        // Free Trial cannot use Image Analyzer (High Load)
        ...
        // 4. Deduct Credits (Ch.5 - Image Analysis = 3 credits)
        const hasCredits = await userService.deductCredits(user.uid, 3);
        if (!hasCredits) {
    ...
    `;
        ...
        // 5. Log Usage
        await logUsage(user.uid, 'image_analysis', 3);
