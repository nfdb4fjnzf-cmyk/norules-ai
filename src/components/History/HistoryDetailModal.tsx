...
const HistoryDetailModal: React.FC<HistoryDetailModalProps> = ({ log, onClose }) => {
    ...
    const result = log.errorMessage || log.resultSummary || (log.outputType ? `Output Type: ${log.outputType}` : '-');
    const cost = log.actualCost !== undefined ? log.actualCost : log.pointsDeducted;
    const mode = log.modelUsed || log.mode || 'UNKNOWN';
    const status = log.status?.toUpperCase() || 'UNKNOWN';
    const tokens = (log.tokensIn || 0) + (log.tokensOut || 0) || log.tokensUsed;
                ...
                <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="bg-black/20 p-3 rounded-xl border border-white/5">
                            ...
                            <div className={`text-sm font-bold ${mode === 'INTERNAL' ? 'text-blue-400' : 'text-green-400'}`}>
                                {mode}
                            </div>
                        </div>
                        <div className="bg-black/20 p-3 rounded-xl border border-white/5">
                            <div className="text-gray-500 text-xs uppercase tracking-wider mb-1">{t('history.details.apiPath')}</div>
                            <div className="text-gray-200 text-sm font-mono">{title}</div>
                        </div>
                        <div className="bg-black/20 p-3 rounded-xl border border-white/5">
                            <div className="text-gray-500 text-xs uppercase tracking-wider mb-1">{t('history.details.status')}</div>
                            <div className={`text-sm font-bold ${status === 'SUCCESS' ? 'text-green-400' : 'text-red-400'}`}>
                                {status}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-4 text-xs text-gray-500 border-t border-white/10 pt-4">
                        {tokens && <div>{t('history.details.tokens')}: <span className="text-gray-300">{tokens}</span></div>}
                        {cost && <div>{t('history.details.points')}: <span className="text-yellow-400">-{cost}</span></div>}
